...
import { compile } from 'src/app/animations';
...
@Component({
  ...
  animations: [compile(size, animationDuration)],
})
export class FieldsComponent implements OnInit, AfterViewChecked {
  ...
  private field: FieldState = new Array(size)
    .fill(null)
      ...
      for (let tileIndex = 0; tileIndex < size; tileIndex++) {
        ...
        if (this.field[rowIndex][tileIndex] !== null) {
          ...
          for (searchIndex = rowIndex; searchIndex < size; searchIndex++) {
            let mergeResult = this.mergeTiles(
              this.field,
              searchIndex,
              tileIndex,
              'down'
        ...
        if (this.field[rowIndex][tileIndex] !== null) {
          ...
          for (searchIndex = tileIndex; searchIndex >= 0; searchIndex--) {
            ...
            if (mergeResult !== false) {
              const diff =
                mergeResult === true
                  ? tileIndex - searchIndex + 1
                  : tileIndex - searchIndex;
              if (diff !== 0) {
                moved = true;
        ...
        if (this.field[rowIndex][tileIndex] !== null) {
          ...
          for (searchIndex = tileIndex; searchIndex < size; searchIndex++) {
            ...
            if (mergeResult !== false) {
              const diff =
                mergeResult === true
                  ? searchIndex - tileIndex + 1
                  : searchIndex - tileIndex;
              if (diff !== 0) {
                moved = true;
  ...
  resetAnimations() {
    this.animations = new Array(size)
      .fill(null)
      .map((_) => new Array(size).fill('base'));
  }

  private initMovement() {
    this.fieldMergeBlock = new Array(size)
      .fill(false)
